...
const titleAndImage = document.querySelector('.title-and-image');
const searchIcon = document.querySelector('svg.search-main-head-icon');
const span = document.querySelector('.hall');
/* console.log(span);
...
function removeModalEvent() {
    searchIcon.removeEventListener('click', openModal);
}
...
svgElement.addEventListener('click', closeModal);
...
function checkWidth() {
    if (window.innerWidth < 910) {
        titleAndImage.appendChild(searchInput);
        titleAndImage.removeChild(searchInput); 
        addModalEvent();
    } else {
        removeModalEvent();
        CreateInput(); 
...
function CreateInput() { 
    ...
    searchInput.classList.add('search-main-head-iconQuery');

    titleAndImage.appendChild(searchInput);
    titleAndImage.appendChild(span); 
}
